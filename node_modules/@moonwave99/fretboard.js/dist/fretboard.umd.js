(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    function getModeFromScaleType(type) {
        const { modeNum } = get$5(type.replace('pentatonic', '').trim());
        return modeNum;
    }
    ...
    const CHROMATIC_SCALE = get$4('C chromatic').notes;
    ...
    class FretboardSystem {
        ...
        getScale({ type = 'major', root: paramsRoot = 'C', box }) {
            ...
            const { notes, empty, intervals } = get$4(scaleName);
            if (empty) {
        ...
        adjustOctave(positions, root) {
            const { tuning } = this;
            const rootOffset = semitones(distance$1(tuning[0], root)) >= 12;
    ...
    function tetrachord({ root, type, layout, string, fret } = {
        ...
        if (layout === exports.TetrachordLayouts.Linear) {
            tetrachord.forEach(x => {
                const { semitones } = get$1(x);
                currentNote = transpose$1(currentNote, x);
                partial += semitones;
                pattern.push({
                    string,
                    fret: fret + partial,
